...
class Blocks extends Component {
  ...
  render() {
            ...
            <Col md={6}>
              ...
              {block.links !== null && (
                <LinkButton
                  ...
                  as="a"
                  href={block.links[0].link}
                  download
                >
                  {block.links[0].linkText}
                </LinkButton>
              )}
              <LinkWrap>
                {block.twitter !== "" && (
    ...
    const returnRenderDesktopButtons = block.node.carouselBlocks.map(
      (block, i) => {
        return this.renderDesktopButtons(i, block);
      }
    );

    const returnRenderMobileButtons = block.node.carouselBlocks.map(
      (block, i) => {
        return this.renderMobileButtons(i, block);
      }
    );
      ...
      <>
        <CarouselWrap>
          ...
          <HexagonCarouselContainer>
            <Row>
              <DesktopHexagons>
                <HexagonCarousel>{returnRenderDesktopButtons}</HexagonCarousel>
              </DesktopHexagons>
              <MobileHexagons>
                <HexagonCarousel>
